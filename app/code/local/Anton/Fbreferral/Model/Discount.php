 ...
 *
 * */
class Anton_Fbreferral_Model_Discount extends Mage_SalesRule_Model_Quote_Discount {
	...
	protected function _aggregateItemDiscount($item)
	{
		...
		$discount_amount = Mage::getModel('fbreferral/fbreferral')->getFbDiscountamount();
		$min_order_amount = Mage::getModel('fbreferral/fbreferral')->getFbOrderamount();
		$enabled = Mage::getModel('fbreferral/fbreferral')->getFbStatus();
	...
	public function fetch(Mage_Sales_Model_Quote_Address $address) {
		...
		if ($amount != 0) {
			...
			if ($discountAmount != $amount) {
				...
				$address->setDiscountDescription('for FB Referral');
				$description = $address->getDiscountDescription();
				$title = Mage::helper('sales')->__('Discount (%s)', $description);
			}
